...
namespace model_model_SEIR_namespace {
...
stan::io::program_reader prog_reader__() {
    ...
    reader.add_event(0, 0, "start", "model_model_SEIR");
    reader.add_event(258, 256, "end", "model_model_SEIR");
    return reader;
}
...
class model_model_SEIR : public prob_grad {
private:
        int nDaySim;
        int mean_DurInf;
        double s_DurInf;
        ...
        int tReopenSchool;
public:
    model_model_SEIR(stan::io::var_context& context__,
    ...
    void ctor_body(stan::io::var_context& context__,
        ...
        static const char* function__ = "model_model_SEIR_namespace::model_model_SEIR";
        ...
        try {
            ...
            current_statement_begin__ = 204;
            context__.validate_dims("data initialization", "s_DurInf", "double", context__.to_vec());
            s_DurInf = double(0);
            vals_r__ = context__.vals_r("s_DurInf");
            pos__ = 0;
            s_DurInf = vals_r__[pos__++];
            ...
            current_statement_begin__ = 205;
            context__.validate_dims("data initialization", "s_DurLat", "double", context__.to_vec());
            s_DurLat = double(0);
            vals_r__ = context__.vals_r("s_DurLat");
            pos__ = 0;
            s_DurLat = vals_r__[pos__++];
    ...
    T__ log_prob(std::vector<T__>& params_r__,
        ...
        try {
            ...
            lp_accum__.add(normal_log<propto__>(R0, mean_R0, s_R0));
            if (R0 < 1) lp_accum__.add(-std::numeric_limits<double>::infinity());
            else if (R0 > 5) lp_accum__.add(-std::numeric_limits<double>::infinity());
            else lp_accum__.add(-log_diff_exp(normal_cdf_log(5, mean_R0, s_R0), normal_cdf_log(1, mean_R0, s_R0)));
            current_statement_begin__ = 233;
            lp_accum__.add(normal_log<propto__>(R0postoutbreak, mean_R0postoutbreak, s_R0postoutbreak));
            if (R0postoutbreak < 0) lp_accum__.add(-std::numeric_limits<double>::infinity());
            else if (R0postoutbreak > 3) lp_accum__.add(-std::numeric_limits<double>::infinity());
            else lp_accum__.add(-log_diff_exp(normal_cdf_log(3, mean_R0postoutbreak, s_R0postoutbreak), normal_cdf_log(0, mean_R0postoutbreak, s_R0postoutbreak)));
            current_statement_begin__ = 234;
            lp_accum__.add(normal_log<propto__>(DurInf, mean_DurInf, s_DurInf));
            if (DurInf < 1) lp_accum__.add(-std::numeric_limits<double>::infinity());
            else if (DurInf > 30) lp_accum__.add(-std::numeric_limits<double>::infinity());
            else lp_accum__.add(-log_diff_exp(normal_cdf_log(30, mean_DurInf, s_DurInf), normal_cdf_log(1, mean_DurInf, s_DurInf)));
            current_statement_begin__ = 235;
            lp_accum__.add(normal_log<propto__>(DurLat, mean_DurLat, s_DurLat));
            if (DurLat < 1) lp_accum__.add(-std::numeric_limits<double>::infinity());
            else if (DurLat > 14) lp_accum__.add(-std::numeric_limits<double>::infinity());
            else lp_accum__.add(-log_diff_exp(normal_cdf_log(14, mean_DurLat, s_DurLat), normal_cdf_log(1, mean_DurLat, s_DurLat)));
    ...
    void write_array(RNG& base_rng__,
        ...
        vars__.resize(0);
        stan::io::reader<local_scalar_t__> in__(params_r__, params_i__);
        static const char* function__ = "model_model_SEIR_namespace::write_array";
        (void) function__;  // dummy to suppress unused var warning
...
typedef model_model_SEIR_namespace::model_model_SEIR stan_model;
