...
rho <- c(rep(0.2, 4), rep(.8, 12)) # probability of being symptomatic per age group
sim_dat <-
  list(
    nDaySim = nDaySim,
    mean_DurInf = 10, #days in I for symptomatic cases
    mean_DurInfsc = 7, #days in I for asymptomatic/subclinical cases
    mean_DurLat = 5, #days in E
    mean_R0 = 2, # mean of R0
    s_R0 = .4, # variance of log(R0)
    mean_R0postoutbreak = 1.5, #mean of R0 postlockdown
    s_R0postoutbreak = .2, #variance of log(R0postlockdown)
